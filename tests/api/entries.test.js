...
describe('GET /users', () => {
  ...
  it('filters entries by week number', async () => {
    ...
    expect(response.data.length).toBeLessThanOrEqual(entriesResponse.data.length);
  });
...
describe('DELETE /entries/:id', () => {
  it('deletes an existing entry', async () => {
    ...
    expect(
      beforeDeleteResponse.data.find(entry => entry.id === newEntryResponse.data.id),
    ).toBeDefined();
    ...
    expect(
      afterDeleteResponse.data.find(entry => entry.id === newEntryResponse.data.id),
    ).toBeUndefined();
  });
});
